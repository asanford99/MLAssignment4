...
# SPECIFICATION: Learns how to classify fashion items
...
for hidden_val in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for neuron_val in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for rate_val in l_rate:                  #looking or the best parameters w.r.t the learning rate

            #build the model for each combination by calling the function:
            #model = build_model()
            #-->add your Pyhton code here
            model = build_model(hidden_val, neuron_val, 10, rate_val)
            ...
            correct = 0
            total = 0
            for (predicted_sample, y_testSample) in zip(class_predicted, y_test):
                total += 1

                if predicted_sample == y_testSample:
                    correct += 1
            ...
            if acc > highestAccuracy:
                ...
                print("Parameters: " + "Number of Hidden Layers: " + str(hidden_val) + ",number of neurons: " + str(neuron_val) + ",learning rate: " + str(rate_val))
                print()
...
print(f'Highest Model Accuracy: {highestAccuracy}')
